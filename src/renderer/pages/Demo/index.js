...
export default class Demo extends Component {
  ...
  componentDidMount() {
    let i = 0;
    setInterval(() => {
      //   const tmpData = this.state.data.pop();
      const tmpData = {
        date: i,
        price: Math.floor(Math.random() * 100),
      };
      const data03 = this.state.data03;
      if (data03.length > 800) {
        data03.shift();
      }
      this.setState(() => {
        return {
          data03: [...data03, tmpData],
          index: data03.length > 100 ? data03.length - 100 : 0,
        };
      });
      i++;
    }, 100);
  ...
  render() {
    const { data03, index } = this.state;
    return (
      <Paper elevation={20} style={{ height: '600', padding: 0 }}>
